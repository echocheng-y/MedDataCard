...
- **RQ3（增量消融）**：LLM 增强相比纯元数据基线，能在不引入编造的前提下把 ST 维度完整度提升多少？（把现有 10.5%→63.4% 实证扩展为可泛化结论）
...
2. **✅ 扩大 M4 抽样验证（已完成）**：已覆盖全部 26/26（130 可比单元，90.0% 精确匹配，κ=0.675，schema 100% 合法）；4 支柱完整度已分报（st_mapping.csv + audit_experiments.csv）。⏳ 第二人类编码员 inter-annotator agreement 待补（当前单编码员）。
...
- M4 扩到 26 数据集、字段精确匹配 90.0%（≥85% ✓）、schema 合法性 100% ✓（⏳ 第二编码员 IAA 待补）
...
2. ✅ 扩大 M4 至 26/26 跨模态抽样（90.0% 精确匹配，κ=0.675）；⏳ 补第二人类编码员 IAA；✅ 4 支柱完整度已分报。
...
- **ST 建议条数口径**：项目 README/PRD 写「18 条记录建议、4 大支柱」，但 STANDING Together 已发表版为 **29 条共识建议**（分 Documentation / Use 两部分）。「18 条」可能是文档子集或早期草稿口径——**在动笔 C1 贡献与 schema 映射前必须先核对**，否则审稿人会直接质疑标准保真度。
  - ✅ **已核对（2026-08）**：ST 已发表版共 **29 条 = 18 Documentation（1.1a–1.4c，可卡片化）+ 11 Use（治理层级）**。项目内部「18 条 / 4 大支柱」即 Documentation 子集，与 29 条不矛盾；schema 与审计均按 18 条 Documentation 对齐，11 条 Use 在文档中明确标注为范围外。
