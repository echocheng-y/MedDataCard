...
**背景** 医学 AI 的数据偏倚以链式方式放大健康不平等；2024 年 STANDING Together（ST）共识首次给出医疗数据集多样性与包容性的统一报告标准，但缺乏可机读、可计算的落地实现，且以大语言模型（LLM）自动抽取元数据存在虚构风险。**目的** 本研究旨在自动生成符合 ST 标准的数据卡，并对公开医学 AI 数据集做系统性、可复现的多样性/包容性审计。**方法** 将 ST 的 18 条文档化建议转译为强约束 JSON Schema（38 字段、6 必填），设计 baseline（仅策展目录、零虚构）+ hybrid（LLM 补全经 schema 护栏校验）双轨抽取管线，并以 26 个旗舰数据集、130 计分单元的人类金标准评测抽取保真度，逐支柱量化合规与多样性。**结果** hybrid 管线抽取准确率达 90.0%（117/130），来源类型 Cohen's κ=0.675；schema 字段级覆盖全部 18 条文档化建议（100%）；逐数据集 ST 四支柱完整度为 63.2 / 54.7 / 79.3 / 59.6%，合规指数 64.2；五指标多样性评分 0.485，其中地理 0.21、肤色 0.12 暴露核心代表性缺口；护栏拦截 14 次越界字段，零虚构逃逸；数据集排名在 200 个 Dirichlet 权重下稳健（Spearman 0.92 / 0.937）。**结论** MedDataCard 首次将 ST 共识落地为机器可校验的数据卡与零虚构审计流水线，量化出医学 AI 数据在地理与肤色维度的系统性代表性缺口，为公平性审计与监管提交提供可复现基础度量。
...
### RQ2 — 标准保真度与合规缺口
schema 在字段级**实现全部 18 条文档化建议（100% 结构覆盖）**；26 张 hybrid 卡的**建议级平均字段覆盖率（来自 `st_mapping.csv`）为 70.3%**（四大支柱分别为 77.6% / 65.4% / 82.1% / 37.2%）。逐数据集口径的**四大支柱完整度（来自 `audit_experiments.csv`，图 2）**为：描述与获取 **63.2%**、人群与地理 **54.7%**、偏倚与标注 **79.3%**、伦理 **59.6%**；复合 ST 合规指数均值 **64.2**（图 2）。两类指标口径不同但互补：前者衡量"schema 是否映射到每条建议"，后者衡量"每张卡在每个支柱上实际填了多少"。26 个数据集的完整度排行榜（图 4）显示跨数据集差异显著（39.1%–76.8%）。数据集多样性评分（五指标均值，图 3）为 **0.485**（0–1）：地理代表性 **0.21**、人群亚组报告 **0.49**、肤色/Fitzpatrick 报告 **0.12**、标注溯源 **0.77**、可泛化性陈述 **0.85**；其中地理（0.21）与肤色（0.12）暴露出核心代表性缺口（逐数据集分布见图 5），标注溯源与可泛化性陈述覆盖较好。与仅用目录的 baseline 相比，hybrid 卡的整体完整度提升约 **+52.9 个百分点**（hybrid ≈63.4% vs baseline ≈10.5%），证明 LLM 补全显著填补了人工目录未覆盖的 ST 字段。最突出的缺口为"随时间的数据漂移"（23.1% 卡覆盖）、"亚组与差异结局"（11.5%）、"患者与公众参与（PPIE）"（0.0%）、"偏倚与影响评估"（11.5%）——这些反映源文档的真实欠报告，而非 schema 遗漏。
...
**局限与下一步（须如实陈述）**：金标准当前为单编码者，需补第二位独立编码者以报告编码者间一致性（IAA）；论文级分层代表性缺口图已生成（图 5）；部分数据集地理/肤色字段为空（含 10 个 baseline 启发式待确认项），相关指标以"未报告"如实计入而非估算；UNI/CONCH/Virchow 等同期 *Nat Med* 2024 范例文献的卷期页须于投稿前终校（MONET 已核验：*Nat Med* 2024;30(4), DOI 10.1038/s41591-024-02887-x）。后续将把可复现的"数据集多样性评分"作为基础度量，接入更广泛的公平性审计（RO6）与监管提交工作流。

---

## 7. 讨论（Discussion）

**代表性缺口的可操作化。** 地理代表性（0.21）与肤色/Fitzpatrick 报告（0.12）两项指标揭示，当前旗舰医学 AI 训练与评估数据的来源高度集中，少数群体与深肤色人群在文档层面几乎不可见。MedDataCard 把这一定性担忧转化为可在数据集间横向比较、可在排行榜上排序的量化指标（图 5 以逐数据集热力图呈现该缺口的结构：SKIN 列近乎全红、GEO 列普遍低位），直接回应 STANDING Together 对"透明披露谁被代表、如何被代表"的核心诉求。
...
**展望**：补全多编码者 IAA（分层代表性缺口图业已生成，见图 5）；将可复现的"数据集多样性评分"作为基础度量，接入更广泛的公平性审计（RO6）与监管提交工作流；并把审计范围从元数据扩展至带许可的数据集原始分布指标。
